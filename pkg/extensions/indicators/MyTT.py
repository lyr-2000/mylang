...
def intvalue(S):
    if isinstance(S, (np.ndarray, list, pd.Series)):
        return S[-1]
    return S
...
def REFX(S, N=1):
    ...
    if isinstance(N, (int, float)):
        ...
        return REFX(S, np.repeat(N, len(S)))
    else:
        N = pd.Series(N)
        S = pd.Series(S)
        ...
        for i in range(len(S)):
            idx = int(N.iloc[i]) + i
            if not np.isnan(N.iloc[i]) and idx >= 0 and idx < len(S):
                res[i] = S.iloc[idx]
        return res
...
def CONST(S):  # 返回序列S最后的值组成常量序列
    S = pd.Series(S)
    return np.full(len(S), S.values[-1])


def sys_to_int(k):
    return k.astype(int).fillna(0)


def Number(S):
    return S.astype(int).fillna(0)


def OR(L, R):
    return sys_is_valid(L) | sys_is_valid(R)


def sys_is_valid(k):
    return (~np.isnan(k)) & (k != 0)
...
def REF(S, N=1):  # 对序列整体下移动N,返回序列(shift后会产生NAN)
    if isinstance(N, (int, float)):
        # return pd.Series(S).shift(N).values
        return REF(S, np.repeat(N, len(S)))
    else:
        N = pd.Series(N)
        S = pd.Series(S)
        res = np.repeat(np.nan, len(S))
        for i in range(len(S)):
            idxRelative = int(N.iloc[i])
            idx = i - idxRelative
            if idx < 0:
                continue
            if idx >= 0 and not np.isnan(N.iloc[i]) and idx >= 0 and idx < len(S):
                res[i] = S.iloc[idx]
        return res
...
def SUM(S, N):  # 对序列求N天累计和，返回序列    N=0对序列所有依次求和
    if isinstance(N, (int, float)):
        return (
            ...
            if N > 0
            else pd.Series(S).cumsum().values
        )
    # N是序列
    res = np.repeat(np.nan, len(S))
    N = pd.Series(N)
    for i in range(len(S)):
        if i >= 0 and i < len(N):
            n = int(N.iloc[i]) if not np.isnan(N.iloc[i]) else 0
            if n > 0:
                res[i] = pd.Series(S).rolling(n).sum().values[i]
            else:
                res[i] = pd.Series(S).cumsum().values[i]
    return res
...
# TODO: beta
def BARSSINCE(S, N=-1):  # N周期内第一次S条件成立到现在的周期数,N为常量  by jqz1226
    if N <= 0:
        N = len(S)
    d = np.zeros(len(S))
    for i in range(len(S)):
...
def HHV(S, N):  # HHV,支持N为序列版本
    ...
    if isinstance(N, (int, float)):
        # return pd.Series(S).rolling(N).max().values
        return HHV(S, np.repeat(N, len(S)))
    else:
        res = np.repeat(np.nan, len(S))
        for i in range(len(S)):
            begin = 0
...
def LLV(S, N):  # LLV,支持N为序列版本
    ...
    # 第一个有效数据点
    if isinstance(N, (int, float)):
        return LLV(S, np.repeat(N, len(S)))
    else:
        res = np.repeat(np.nan, len(S))
        for i in range(len(S)):
            if N[i] == 0:
...
def DSMA(X, N):  # 偏差自适应移动平均线   type: (np.ndarray, int) -> np.ndarray
    ...
    for i in range(len(X)):
        Filt[i] = (
            c1 * (Zeros[i] + Zeros[i - 1]) / 2 + c2 * Filt[i - 1] + c3 * Filt[i - 2]
        )
...
def SUMBARSFAST(X, A):
    ...
    if isinstance(A * 1.0, float):
        A = np.repeat(A, length)  # 是单值则转化为数组
    A = np.flipud(A)  # 倒转
    sumbars = np.zeros(length)  # 初始化sumbars为0
    Sigma = np.insert(
        np.cumsum(X), 0, 0.0
    )  # 在累加值前面插入一个0.0（元素变多1个，便于引用）
...
def SAR(HIGH, LOW, N=10, S=2, M=20):
    ...
    for i in range(N, length):
        ...
        if (is_long and LOW[i] < sar_x[i]) or (
            (not is_long) and HIGH[i] > sar_x[i]
        ):  # 反空 或者 反多
            is_long = not is_long
            b_first = True
    return sar_x
